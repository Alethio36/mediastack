...
cmd_doctor() {
    ...
    if compgen -G "local/recyclarr/recyclarr.yml" >/dev/null; then
...
trash_gen_config() {
    local out=local/recyclarr/recyclarr.yml
    mkdir -p local/recyclarr
    {
    ...
    for s in $(trash_instances); do
        ...
        if [[ "$app" == sonarr && $wrote_sonarr == 0 ]]; then echo "sonarr:" >> "$out"; wrote_sonarr=1; fi
        if [[ "$app" == radarr && $wrote_radarr == 0 ]]; then echo "radarr:" >> "$out"; wrote_radarr=1; fi
        {
            echo "  $s:"
