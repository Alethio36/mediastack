...
| `upgrade` | mediastack itself: git pull + `.env` migration, run by the freshly pulled code; then says whether `up` has anything to apply, naming the services it would change (Docker's own comparison — a comment edit never triggers it). Images stay put — that's `update` |
