...
Prefer a GUI, or no Cloudflare domain? `enable npm` ships Nginx Proxy
Manager instead: same 80/443, proxy hosts and certificates managed in its
own UI at `http://<host>:81`, nothing generated by the stack. The two
publish the same ports, so `enable` refuses the second while the first is
on — pick one edge.
