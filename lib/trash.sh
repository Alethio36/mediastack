...
trash_gen_config() { # trash_gen_config [out-path] — default: the live recyclarr.yml
    ...
    tmp=$(mktemp)
    {
        ...
        echo "# custom/trash-overrides.yml. Regenerated on every trash-sync run."
...
TRASH_SENTINEL='[!] Synced by mediastack — tune via custom/trash-overrides.yml'
TRASH_SENTINEL_BEFORE='[!] Synced by mediastack — tune via local/trash-overrides.yml'   # its name before schema 28
trash_sentinel() { # $1 = svc
    local s="$1" key url cur old
    ...
    old=$(jq -c --arg n "$TRASH_SENTINEL_BEFORE" 'first(.[] | select(.name == $n)) // empty' <<<"$cur")
    if jq -e --arg n "$TRASH_SENTINEL" 'any(.[]; .name == $n)' <<<"$cur" >/dev/null 2>&1; then
        ok "$s: sentinel banner present"
    elif [[ -n "$old" ]]; then   # renamed in place: the same custom format, so profiles keep it
        if w_would "$s: rename the sentinel banner to point at custom/trash-overrides.yml"; then
            local rresp
            if rresp=$(api PUT "$url/api/$(arr_apiver "$s")/customformat/$(jq -r .id <<<"$old")" "$key" \
                "$(jq -c --arg n "$TRASH_SENTINEL" '.name = $n' <<<"$old")"); then
                ok "$s: sentinel banner renamed"
            else
                wfail "$s: sentinel rename rejected — API said: $(head -c160 <<<"$rresp")"
            fi
        fi
